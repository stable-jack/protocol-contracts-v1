...
contract ChainlinkPriceOracle is Ownable, IPriceOracle {
  ...
  function price(address _token) external view override returns (uint256) {
    ...
    (uint80 roundID, int256 _answer, ,uint256 updatedAt, uint80 answeredInRound) = AggregatorV3Interface(_info.feed).latestRoundData();

    require(roundID != 0, "Stale price");
    require(answeredInRound >= roundID, "Stale price"); 
    require(updatedAt != 0, "Round not complete");
    require(updatedAt <= block.timestamp, "Round not complete");
