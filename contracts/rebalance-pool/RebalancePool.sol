...
contract RebalancePool is OwnableUpgradeable, IJackRebalancePool {
  ...
  /// @param index The address of the token.
  /// @param token The manager of the reward token.
  event RemoveRewardToken(uint256 index, address token);
  ...
  /// @notice Emitted when rewards are distributed for a token.
  /// @param token The address of the token for which rewards are distributed.
  /// @param duration The duration for which rewards are calculated.
  /// @param pending The amount of pending rewards.
  /// @param accumulated Indicates whether the rewards were accumulated or queued.
  ...
  /// @notice Emitted when rewards are accumulated for a token.
  /// @param token The address of the token for which rewards are accumulated.
  /// @param pending The amount of pending rewards.
  /// @param rewardsPerUnitStaked The calculated rewards per unit staked.
  /// @param epoch The current epoch.
  /// @param scale The current scale.
  /// @param newRewardSum The updated reward sum after accumulation.
  event RewardsAccumulated(
    ...
    uint256 epoch,
    uint256 scale,
    uint256 newRewardSum
  );

  /// @notice Emitted when an account snapshot is taken.
  /// @param account The address of the account for which the snapshot is taken.
  /// @param newDeposit The new deposit amount.
  /// @param newUnlock The new unlock amount.
  /// @param epoch The current epoch during which the snapshot is taken.
  ...
  /// @notice Emitted when a loss is notified and handled.
  /// @param loss The amount of loss.
  /// @param baseOut The base output amount.
  /// @param assetLossPerUnitStaked The calculated loss per unit staked.
  /// @param epoch The current epoch.
  /// @param scale The current scale.
  /// @param newProductFactor The new product factor after the loss.
  event LossNotified(
    uint256 loss,
    uint256 baseOut,
  ...
  function initialize(address _treasury, address _market) external initializer {
    ...
    asset = IJackTreasury(_treasury).aToken();
  ...
  function unlock(uint256 _amount) external override {
    ...
    _unlock.amount = _amount.add(_unlock.amount);
    ...
    snapshots[msg.sender].initialUnlock.unlockAt = _unlockAt;
    ...
    returns (uint256 _liquidated, uint256 _baseOut)
  {
    ...
    (, uint256 _maxLiquidatable) = _treasury.maxRedeemableaToken(liquidatableCollateralRatio);
    ...
    (_baseOut, ) = IJackMarket(_market).redeem(_amount, 0, _wrapper, _minBaseOut);
  ...
  function removeReward(uint256 _index) external onlyOwner {
    ...
    require(extraRewardState[_token].queued == 0, "has undistributed rewards");
    require(extraRewardState[_token].finishAt < block.timestamp, "has undistributed rewards");
  ...
  function _distributeReward(address _token) internal {
    ...
    if (_duration > 0) {
        ...
        emit RewardsDistributed(_token, _duration, _pending, _accumulated);
    }
  }
  ...
  function _updateAccountRewards(address _account) internal {
    ...
    if (_initialUnlock > 0) {
      ...
      if (_compoundedUnlock != _initialUnlock) {
        ...
        snapshots[_account].initialUnlock.amount = _compoundedUnlock;
      }
    }
  ...
  /// @param _account The address of user to update.
  function _takeAccountSnapshot( address _account, uint256 _newDeposit, uint256 _newUnlock) internal {
      snapshots[_account].initialDeposit = _newDeposit;
      snapshots[_account].initialUnlock.amount = _newUnlock;

      if (_newDeposit == 0 && _newUnlock == 0) {
          delete snapshots[_account].epoch;
          
          // Emit the event
          emit AccountSnapshotTaken(_account, _newDeposit, _newUnlock, 0);
          return;
      }
      ...
      if (_newDeposit != 0) {
        ...
        for (uint256 i = 0; i < length; i++) {
          address _token = extraRewards[i];
          snapshots[_account].extraRewards[_token].accRewardsPerStake = epochToScaleToExtraRewardSum[_token][_currentEpoch.epoch][_currentEpoch.scale];
        }
      }
      ...
      emit AccountSnapshotTaken(_account, _newDeposit, _newUnlock, _currentEpoch.epoch);
  }
  ...
  function _accumulateRewards(address _token, uint256 _pending) internal {
      ...
      emit RewardsAccumulated(_token, _pending, _rewardsPerUnitStaked, _currentEpoch.epoch, _currentEpoch.scale, _currentSum);
  }
  ...
  function _notifyLoss(uint256 _loss, uint256 _baseOut) internal {
    ...
    // use >= here, in case someone sends extra assets to this contract.
    ...
    emit LossNotified(_loss, _baseOut, _assetLossPerUnitStaked, _currentEpoch.epoch, _currentEpoch.scale, _newProductFactor);
  }
  ...
  function _claim(
    ...
    if(_unwrap) {
      IButtonToken(baseToken).burnTo(_account, _rewards);
    }
